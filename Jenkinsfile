   node {
      ...
      stage('deploy') {
        withCredentials([azureServicePrincipal('azure_service_principal')]) {
          ...
          sh 'az spring app deploy -n gs-demo -s cicd-service -g jenkins --jar-path /var/lib/jenkins/workspace/Deploy-DemoApp/complete/target/spring-boot-complete-0.0.1-SNAPSHOT.jar'
          sh 'az logout'
        }
      }
    }
